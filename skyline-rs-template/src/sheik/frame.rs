...
unsafe extern "C" fn sheik_on_main(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if !wallJumpUsed{
            ...
            || status_kind == *FIGHTER_SHEIK_STATUS_KIND_SPECIAL_LW_ATTACK || status_kind == *FIGHTER_SHEIK_STATUS_KIND_SPECIAL_LW_RETURN
            {
                if situation_kind == *SITUATION_KIND_AIR{
...
pub fn install(agent: &mut smashline::Agent) {
    agent.on_line(smashline::Main, sheik_on_main);
}