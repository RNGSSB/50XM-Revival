...
unsafe extern "C" fn char_on_main(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if !wallJumpUsed{
        if status_kind == *FIGHTER_STATUS_KIND_SPECIAL_S || status_kind == *FIGHTER_LUIGI_STATUS_KIND_SPECIAL_S_WALL
        || status_kind == *FIGHTER_LUIGI_STATUS_KIND_SPECIAL_S_RAM || status_kind == *FIGHTER_LUIGI_STATUS_KIND_SPECIAL_S_END
        {
            if situation_kind == *SITUATION_KIND_AIR{
