...
unsafe extern "C" fn falcon_on_main(fighter: &mut L2CFighterCommon) {
    unsafe {
        ...
        if !wallJumpUsed{
            if status_kind == *FIGHTER_STATUS_KIND_SPECIAL_S || status_kind == *FIGHTER_STATUS_KIND_SPECIAL_LW
            || status_kind == *FIGHTER_CAPTAIN_STATUS_KIND_SPECIAL_LW_END {
                if situation_kind == *SITUATION_KIND_AIR{
...
pub fn install(agent: &mut smashline::Agent) {
    agent.on_line(smashline::Main, falcon_on_main);
}