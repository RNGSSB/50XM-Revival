...
pub unsafe fn shieldDrops(boma: &mut smash::app::BattleObjectModuleAccessor, status_kind: i32, cat2: i32 , cat1: i32)  {
    if status_kind == *FIGHTER_STATUS_KIND_GUARD || status_kind == *FIGHTER_STATUS_KIND_GUARD_ON  { //Shield Drop
        if (cat2 & *FIGHTER_PAD_CMD_CAT2_FLAG_GUARD_TO_PASS != 0 && cat1 & *FIGHTER_PAD_CMD_CAT1_FLAG_SPECIAL_ANY != 0) || (cat2 & *FIGHTER_PAD_CMD_CAT2_FLAG_APPEAL_HI) != 0 || (cat2 & *FIGHTER_PAD_CMD_CAT2_FLAG_APPEAL_LW) != 0  ||
        ...
        *FIGHTER_STATUS_KIND_APPEAL, *FIGHTER_STATUS_KIND_ESCAPE_AIR, *FIGHTER_STATUS_KIND_ESCAPE_AIR_SLIDE, *FIGHTER_STATUS_KIND_LANDING_ATTACK_AIR].contains(&status_kind) {
        ...
        if situation_kind == *SITUATION_KIND_GROUND {
            if status_kind == *FIGHTER_STATUS_KIND_SPECIAL_HI { //Up specials
                if [*FIGHTER_KIND_PIKACHU, *FIGHTER_KIND_GEKKOUGA].contains(&fighter_kind) {
                    fix = *GROUND_CORRECT_KIND_GROUND as u32;
                }
            }
        }
...
pub unsafe fn quickAttackCancels (boma: &mut smash::app::BattleObjectModuleAccessor, status_kind: i32, situation_kind: i32, fighter_kind: i32, stick_value_y: f32) {
    if [*FIGHTER_KIND_PIKACHU].contains(&fighter_kind) {
        if [*FIGHTER_PIKACHU_STATUS_KIND_SPECIAL_HI_WARP].contains(&status_kind) {
            if situation_kind == *SITUATION_KIND_GROUND {
...
pub unsafe extern "C" fn global_fighter_frame(fighter : &mut L2CFighterCommon) {
    ...
    quickAttackCancels(module_accessor, status_kind, situation_kind, fighter_kind, stick_value_y);
}
